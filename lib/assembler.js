...
var mi = (I, O, V) => [m8816(I, O, V)];
// simple op: single 32bit number specifies 16bit instruction, and a 16bit number
//           for value.
var mi1 = (I, V)   => [m8816(I, 0, V)];
// TODO: WRONG
// extended op: single 32bit number specifies 8bit instruction, 8bit register (if any),
//           an 8bit value, and a 32bit secondary value
var mi2 = (I, O, V1, V2) => [mi(I, O, V1), n(V2)]
var bits = (N) => N.toString(2)

var _nop = () => mi(ops.nop, 0x0, 0x0)
var _lrl = (Reg, Lit) => mi(ops.lrl, Reg, Lit)
var _lra = (Reg, Adr) => mi(ops.lra, Reg, Adr)
var _rlr = (R0, R1)   => mi(ops.rlr, R0,  R1)
var _in  = (Port)     => mi(ops.in,  Port, 0x0)
var _out = (Port, Reg)=> mi(ops.out, Reg, Port)
var _push= (Value)    => mi(ops.psh, Value, 0x0)
var _pop = (Reg)      => mi(ops.pop,  Reg, 0x0)
var _peek= (Reg)      => mi(ops.peek, Reg, 0x0)
var _stle= ()         => mi(ops.stle, 0x0, 0x0)
var _jcmp= (WhichReg, Location) =>
    mi(ops.jcmp, WhichReg, Location)
var _xor = (Reg, Mask)=> mi(ops.xor, Reg, Mask)
var _halt= ()         => mi(ops.ext, ops.halt, 0)
var _stat= ()         => mi(ops.ext, ops.stat, 0)
var _idis= ()         => mi(ops.int, ops.idis, 0x0 << 8)
var _iena= ()         => mi(ops.int, ops.iena, 0x0 << 8)
var _istp= (Id, Reg)  => mi(ops.int, ops.istp, (Id << 8) | Reg)
var _idel= (Id)       => mi(ops.int, ops.del,  (Id << 8))
var _int=  (Id)       => mi(ops.int, ops.iint, (Id << 8))
var ins = {};
ins.nop = _nop;
ins.lrl = _lrl;
ins.lra = _lra;
...
function compile (c) {
	// Refresh opcode symbols
	ops = symbols.getSymbols().ops;
	lines = c.split("\n");
